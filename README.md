...
- **Multi-Machine Runners**: Install Go runners on any development machine. The UI lets you pick which runner runs each session. Administrators can deliberately disconnect a runner (stopping its active tasks) and delete disconnected runners from the Runners dashboard.
...
- **autoagent (Auto Mode)**: Automatically selects the best agent from known subscription quotas, preferring available capacity over unknown data and using API-key billed choices only as a last fallback. When configured, automodel makes a second-stage model/effort choice from only the quota-filtered options that Auto Mode passes to it.
...
Use `--once` with `--temp-dir` to create a single-use session that disallows follow-up messages; subsequent `--resume` or `--session-id` requests targeting this session will be rejected.
Use `--cache on` (default: `off`, requires `--once`) to cache single-use session outputs in `${ARONDO_CONFIG_DIR}/cache/once/` keyed by the SHA-256 hash of the prompt; subsequent matching calls return the cached result immediately without invoking an agent.
