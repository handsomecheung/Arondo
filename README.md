...
- **Task Queue & Live Tracking**: Active task queue in the header with PID tracking and live log inspection. Clicking a task opens its dedicated console log modal. Each running task can be killed from the queue, and stopped project-scoped tasks can be deleted from their three-dot menu. Completed tasks are retained for 3 days, and are removed from the queue as soon as their session is archived. Tasks and todo messages belonging to hidden projects are automatically filtered out from the active queue and Tasks dashboard.
