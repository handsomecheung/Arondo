...
- **Session Pinning, Filtering & Three-dot Menu**: Pin important sessions to the top of the sidebar session list (ordered by pinned timestamp). When sessions span multiple projects or runners, horizontally scrollable filter chips appear in the sidebar to let users filter sessions by project or runner. A per-session three-dot menu in the sidebar and detailed session view provides quick access to Pin, Rename, Archive, and Delete actions.
...
- **Task Queue & Live Tracking**: Active task queue in the header with PID tracking and live log inspection. Clicking a task opens its dedicated console log modal. Each running task can be killed from the queue, and stopped project-scoped tasks can be deleted from their three-dot menu. Completed tasks are retained for 3 days, and are removed from the queue as soon as their session is archived.
