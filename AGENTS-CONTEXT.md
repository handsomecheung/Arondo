...
app/
  ...
  api/
    ...
    tasks/
      route.ts        # GET: list all tasks (active + retained, excluding tasks belonging to hidden projects or archived sessions)
      kill/
        route.ts        # POST: kill a running task by sessionId + messageId
    todo-messages/
      route.ts          # GET: flat list of every session's pending todo messages (read-time join, excluding those belonging to hidden projects; backs the Tasks dashboard "Upcoming" section)
