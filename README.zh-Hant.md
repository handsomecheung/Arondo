...
配合 `--temp-dir` 使用 `--once` 可建立禁止後續訊息的一次性會話；針對該會話的後續 `--resume` 或 `--session-id` 請求將被拒絕。
使用 `--cache on`（預設值：`off`，需要配合 `--once`）將一次性會話輸出按 Prompt 的 SHA-256 雜湊作為 Key 快取在 `${ARONDO_CONFIG_DIR}/cache/once/` 中；後續相符的調用將直接回傳快取結果而無需調用 Agent。
